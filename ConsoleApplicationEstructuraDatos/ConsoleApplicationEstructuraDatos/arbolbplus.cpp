...
public:
	...
	bool buscar(int clave) {
		...
		return binary_search(actual->claves.begin(), actual->claves.end(), clave);
	}
...
private:
	...
	void revisequenoestecreado(Nodo* nodo, int clave) {
		if (nodo->esHoja) {
			nodo->claves.insert(
				upper_bound(nodo->claves.begin(), nodo->claves.end(), clave), clave);
		}
		else {
			int i = 0;
			...
			revisequenoestecreado(nodo->hijos[i], clave);
		}
